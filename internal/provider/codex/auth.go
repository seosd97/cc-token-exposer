...
var ErrAPIKeyMode = &provider.NoPlanError{Reason: "codex: logged in with an API key; plan limits do not apply"}
...
func (s *AuthSource) Load() (*provider.Credentials, error) {
	path := s.Path
	if path == "" {
...
func parseAuthJSON(data []byte) (*provider.Credentials, error) {
	var af authFile
	...
	c := &provider.Credentials{AccessToken: token, AccountID: accountID}
