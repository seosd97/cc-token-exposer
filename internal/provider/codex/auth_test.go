...
func TestAuthSourceLoad(t *testing.T) {
	...
	if _, err := (&AuthSource{Path: filepath.Join(dir, "missing.json")}).Load(); !errors.Is(err, provider.ErrNotAvailable) {
...
func TestAuthSourceResolvesThroughTheCredsResolver(t *testing.T) {
	...
	c, err := provider.NewResolver(&AuthSource{Path: path}).Resolve()
