...
var testCreds = &provider.Credentials{AccessToken: testToken, AccountID: "acct-test"}
...
func fetch(t *testing.T, status int, body string) (*provider.FetchedSnapshot, *http.Request, error) {
	t.Helper()
	var got *http.Request
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		got = r.Clone(context.Background())
...
func TestFetchOmitsAccountHeaderWhenUnknown(t *testing.T) {
	...
	if _, err := c.Fetch(context.Background(), &provider.Credentials{AccessToken: testToken}); err != nil {
		t.Fatalf("Fetch: %v", err)
	}
...
func TestFetchStatusMapping(t *testing.T) {
	...
	if !errors.As(err, &rl) || !errors.Is(err, provider.ErrRateLimited) || rl.RetryAfter != 45*time.Second {
...
func TestFetchRejectsEmptyToken(t *testing.T) {
	c := New()
	for _, cr := range []*provider.Credentials{nil, {}, {AccountID: "acct"}} {
		if _, err := c.Fetch(context.Background(), cr); !errors.Is(err, provider.ErrAuth) {
...
func TestFetchErrorsNeverCarryTheToken(t *testing.T) {
	...
	_, err = c.Fetch(context.Background(), testCreds)
	if err == nil || strings.Contains(err.Error(), testToken) || !errors.Is(err, provider.ErrTransient) {
